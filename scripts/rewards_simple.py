...
vault_registry.deposit_collateral(vault1, 1000)  # = 500_000 USD capacity
vault_registry.deposit_collateral(vault2, 1000)  # = 250_000 USD capacity
vault_registry.deposit_collateral(vault3, 2000)  # = 1_000_000 USD capacity

for vault in [vault1, vault2, vault3]:
    print(f"Vault capacity is {vault_registry.vault_capacity(vault)} USD")

vault_registry.distribute(10)
print(
    "DOT rewards: ",
    vault_registry.compute_reward(vault1),
    vault_registry.compute_reward(vault3),
)
print("KSM rewards: ", vault_registry.compute_reward(vault2), "\n")
